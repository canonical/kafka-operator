...
In order to set up secure connection Apache Kafka and Apache ZooKeeper applications need to be integrated with TLS Certificate Provider charms, e.g. 
...
both in an Apache ZooKeeper zNode and in a JAAS configuration file in the Apache Kafka server in plain format. 
