...
We are extremely thrilled and excited to share that Charmed Apache Kafka and Charmed ZooKeeper have now been released as GA. You can find them in [charmhub.io](https://charmhub.io/) under the `3/stable` track.
...
* Inplace upgrade for charms tracking `latest` is not supported, both for ZooKeeper and Kafka charms. Perform data migration to upgrade to a Charmed Apache Kafka cluster managed via a `3/stable` charm. 
