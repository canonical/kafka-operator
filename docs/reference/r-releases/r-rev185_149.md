...
We are glad to report that we have just released a new updated version for the Charmed Kafka bundles on the `3/stable` channel, 
upgrading Charm ZooKeeper revision from 136 to 149, and Charmed Kafka from xxx to 188.  
...
Here below we provide a summary of the main features and bug fixes related to the Kafka operator.
