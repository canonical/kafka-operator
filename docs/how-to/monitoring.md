...
Now, deploy `grafana-agent` (subordinate charm) and relate it with Charmed Apache Kafka:
...
To tune the server log level for Apache Kafka, adjust the `log-level` configuration.

For Charmed Apache Kafka, set the `log-level` parameter:
