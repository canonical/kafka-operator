...
To remove Charmed Apache Kafka and the `tutorial` model it is hosted on, along with all other
applications:
...
If you don't need Juju anymore and want to free up additional resources on your machine, you can
remove the Juju controller and Juju itself.
...
In this tutorial, we've successfully deployed Apache Kafka, added/removed replicas, added/removed
users to/from the cluster, and even enabled and disabled TLS. You may now keep your Charmed Apache
Kafka deployment running or remove it entirely using the steps in
[Remove Charmed Apache Kafka and Juju](remove-kafka-and-juju). If you're looking for what to do next
you can:

- Run [Charmed Apache Kafka on Kubernetes](https://github.com/canonical/kafka-k8s-operator).
- Check out our other Charmed offerings from
