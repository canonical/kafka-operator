...
Make sure that the `kafka-test-app` is not connected to the Charmed Apache Kafka, by removing the
...
Then, enable encryption on the `kafka-test-app` by relating with the `self-signed-certificates`
charm:
...
there is no difference here with the unencrypted workflow):
...
As before, you can check that the messages are pushed into the Charmed Apache Kafka cluster by
inspecting the logs:
...
Note that if the `kafka-test-app` was running before, there may be multiple logs related to the
different runs. Refer to the latest logs produced and also check that in the logs the connection is
indeed established with the encrypted port `9093`.
