...
from literals import CONTAINER, INTERNAL_USERS, PEER, SUBSTRATE
...
def test_set_unset_tls_private_key(ctx: Context, base_state: State):
    secret_content = {}
    for broker_id in range(3):
        tls_artifacts = generate_tls_artifacts(
            subject=f"kafka/{broker_id}",
            sans_ip=["10.10.10.10"],
            sans_dns=[f"kafka/{broker_id}"],
            with_intermediate=False,
        )

        secret_content.update({broker_id: tls_artifacts.private_key})
    ...
    with ctx(ctx.on.secret_changed(tls_private_key_secret), state_in) as mgr:
        charm: KafkaCharm = cast(KafkaCharm, mgr.charm)
        _ = mgr.run()

        assert charm.state.unit_broker.client_certs.private_key == secret_content[0]

        secret_private_key = charm.tls.certificates._private_key
        assert secret_private_key == PrivateKey.from_string(secret_content[0])
    ...
    with ctx(ctx.on.secret_changed(tls_private_key_secret), state_interim) as mgr:
        charm: KafkaCharm = cast(KafkaCharm, mgr.charm)
        _ = mgr.run()

        assert not charm.state.unit_broker.client_certs.private_key

        default_private_key = charm.tls.certificates._private_key
        assert (
            default_private_key != secret_private_key
        )  # key actually changed back to autogenerated
