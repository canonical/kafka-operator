...
def sign_manual_certs(model: str, manual_app: str = "manual-tls-certificates") -> None:
    ...
    for i, csr in enumerate(csrs):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                sign_cmd = f"openssl x509 -req -in {csr_file} -CAkey tests/integration/k8s/data/int.key -CA tests/integration/k8s/data/int.pem -days 100 -CAcreateserial -out {cert_file} -copy_extensions copyall --passin pass:password"
                provide_cmd = f'JUJU_MODEL={model} juju run {manual_app}/0 provide-certificate ca-certificate="$(base64 -w0 tests/integration/k8s/data/int.pem)" ca-chain="$(base64 -w0 tests/integration/k8s/data/root.pem)" certificate="$(base64 -w0 {cert_file})" certificate-signing-request="$(base64 -w0 {csr_file})"'
