...
from integration.connect_k8s.helpers import DatabaseFixtureParams
...
@pytest.fixture(scope="module")
def integrator_charm(juju: JujuFixture):
    """Build the integrator charm."""
    charm_path = "./tests/integration/connect_k8s/app-charm/"
    charm = juju.ext.build_charm(charm_path, use_cache=bool(os.environ.get("CI")))
    return charm
...
def mysql_test_data(juju: JujuFixture, request: pytest.FixtureRequest):
    ...
    def exec_query(query: str):
        ...
        return_code, a, b = juju.juju("ssh", "--container", "mysql", f"{mysql_leader.name}", cmd)
