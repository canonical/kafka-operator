...
from integration.machine.ha.continuous_writes import ContinuousWritesResult
from integration.machine.helpers import APP_NAME, CONTROLLER_NAME, KRaftMode
from integration.machine.helpers.jubilant import (
    KRaftUnitStatus,
    all_active_idle,
    kraft_quorum_status,
)
from integration.machine.helpers.pytest_operator import check_socket, get_unit_ipv4_address
...
def get_kraft_quorum_lags(juju: jubilant.Juju) -> list[int]:
    ...
    for unit in juju.status().apps[CONTROLLER_NAME].units:
        try:
            ...
            bootstrap_controller = f"{unit_ip}:{CONTROLLER_PORT}"
            result = check_output(
                f"JUJU_MODEL={juju.model} juju ssh {unit} sudo -i 'charmed-kafka.metadata-quorum  --command-config {PATHS['kafka']['CONF']}/kraft-client.properties --bootstrap-controller {bootstrap_controller} describe --replication'",
                stderr=PIPE,
                shell=True,
                universal_newlines=True,
            )
