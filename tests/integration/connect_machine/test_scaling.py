...
def test_deploy_integrator(juju: JujuFixture, source_integrator_charm):
...
def test_destroy_active_workers(juju: JujuFixture):
    ...
    """
    while len(juju.ext.model.applications[APP_NAME].units) > 1:
    ...
    } == {get_unit_ipv4_address(juju, remaining_unit)}
