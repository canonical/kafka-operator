...
from literals import TLS_RELATION, TRUSTED_CA_RELATION, TRUSTED_CERTIFICATE_RELATION, PEER
from utils import (
    parse_tls_file,
)
from ..common.charm import PeerRelationMixin
from new.core.models import CharmState
from new.managers.tls import CertificatesManager
from new.core.workload import AbstractKafkaService

logger = logging.getLogger(__name__)


class TLSHandlers(PeerRelationMixin):
    ...
    def __init__(
        ...
        self.certificates_manager = CertificatesManager(state.config, self.cluster, workload)

        # self.certificates_manager = certificates_manager
    ...
    def _tls_relation_created(self, _) -> None:
        ...
        if not self.charm.unit.is_leader() or not self.cluster.tls.enabled:
            return

        self.app_peer_data.update({"tls": "enabled"})
    ...
    def _tls_relation_joined(self, _) -> None:
        ...
        if not self.cluster.tls.private_key:
            self.set_secret(
                scope="unit", key="private-key",
                value=generate_private_key().decode("utf-8")
            )

        # generate unit private key if not already created by action
        if not self.cluster.tls.keystore_password:
            self.set_secret(
                scope="unit", key="keystore-password",
                value=self.certificates_manager.generate_password()
            )
        if not self.cluster.tls.truststore_password:
            self.set_secret(
                scope="unit", key="truststore-password",
                value=self.certificates_manager.generate_password()
            )

        """Generates and submits CSR to provider."""
        if not (private_key := self.cluster.tls.private_key):
            logger.error("Can't request certificate, missing private key")
            return

        csr = generate_csr(
            private_key=private_key.encode("utf-8"),
            subject=self.cluster.unit.address,
            sans_ip=self.certificates_manager.sans["sans_ip"],
            sans_dns=self.certificates_manager.sans["sans_dns"],
        )

        self.set_secret(scope="unit", key="csr", value=csr.decode("utf-8").strip())
    ...
    def _tls_relation_broken(self, _) -> None:
        ...
        self.certificates_manager.remove_stores()

        if not self.charm.unit.is_leader():
            return

        self.app_peer_data.update({"tls": "disabled"})
        self.set_secret(scope="unit", key="csr", value="")
        self.set_secret(scope="unit", key="certificate", value="")
        self.set_secret(scope="unit", key="ca", value="")
    ...
    def _trusted_relation_created(self, _) -> None:
        ...
        if not self.cluster.tls.enabled:
        ...
        self.app_peer_data.update({"mtls": "enabled"})
    ...
    def _trusted_relation_joined(self, event: RelationJoinedEvent) -> None:
        ...
        alias = self.certificates_manager.generate_alias(
        ...
        csr = (
            generate_csr(
                ...
                private_key=self.cluster.tls.private_key.encode(
    ...
    def _trusted_relation_changed(self, event: RelationChangedEvent) -> None:
        ...
        relation_data = _load_relation_data(dict(
            event.relation.data[event.relation.app]))  # type: ignore[reportOptionalMemberAccess]
        ...
        self.certificates_manager.import_cert(alias=f"{alias}", content=content)
    ...
    def _trusted_relation_broken(self, event: RelationBrokenEvent) -> None:
        ...
        self.certificates_manager.remove_cert(alias=alias)
        ...
        if not all_relations:
            self.app_peer_data.update({"mtls": "disabled"})
    ...
    def _on_certificate_available(self, event: CertificateAvailableEvent) -> None:
        ...
        if event.certificate_signing_request != self.cluster.tls.csr:
            logger.error("Can't use certificate, found unknown CSR")
            return

        self.set_secret(scope="unit", key="certificate", value=event.certificate)
        self.set_secret(scope="unit", key="ca", value=event.ca)

        self.certificates_manager.set_server_key()
        self.certificates_manager.set_ca()
        self.certificates_manager.set_certificate()
        self.certificates_manager.set_truststore()
        self.certificates_manager.set_keystore()

    def _on_certificate_expiring(self, _) -> None:
        """Handler for `certificate_expiring` event."""
        if not (private_key := self.cluster.tls.private_key) or not (csr := self.cluster.tls.csr):
        ...
        self.certificates.request_certificate_renewal(
            old_certificate_signing_request=csr.encode("utf-8"),
            new_certificate_signing_request=new_csr,
        )

        self.set_secret(scope="unit", key="csr", value=new_csr.decode("utf-8").strip())
    ...
    def _set_tls_private_key(self, event: ActionEvent) -> None:
        ...
        self.set_secret(scope="unit", key="private-key", value=private_key)
